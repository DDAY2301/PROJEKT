(() => {
  ...
  function ensureStyles() {
    ...
    style.textContent = `
      .pv-preview-modal{position:fixed;inset:0;z-index:9999;display:none;background:rgba(3,18,14,.78);backdrop-filter:blur(10px);padding:3vh 3vw}.pv-preview-modal.open{display:grid;grid-template-rows:auto 1fr}.pv-preview-bar{display:flex;align-items:center;justify-content:space-between;gap:1rem;background:#071d17;color:#fff;border-radius:18px 18px 0 0;padding:.8rem 1rem}.pv-preview-bar strong{font-size:.9rem}.pv-preview-bar span{color:#a7bdb4;font-size:.7rem}.pv-preview-close{border:1px solid rgba(255,255,255,.22);background:transparent;color:#fff;border-radius:999px;padding:.45rem .7rem;cursor:pointer}.pv-preview-frame{width:100%;height:100%;border:0;background:#fff;border-radius:0 0 18px 18px}.pv-preview-loading{display:grid;place-items:center;background:#fff;color:#23463c;border-radius:0 0 18px 18px;font-weight:750}.pv-handoff-note{font-size:.65rem;color:var(--muted);margin-top:.4rem}
    `;
  ...
  function ensureModal() {
    ...
    const close = () => { modal.classList.remove('open'); const frame=document.getElementById('pvPreviewFrame'); if(frame){frame.src='about:blank';frame.style.display='none';} const load=document.getElementById('pvPreviewLoading'); if(load) load.style.display='grid'; };
